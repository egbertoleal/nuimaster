...
{
    public static class AdaptedReduction
    {
        ...
        private const string REDUCTION_ALGORITHM = ConstAux.REDUCTION_ALGORITHM_ADAPTED;
        ...
        public static void RunReduction(string projectName, string clusteringAlgorithmName, int numClusters)
        {
            ...
            foreach (var clusterGroup in groupedByCluster)
            {
                ...
                var groupedByDistance = clusterGroup.Items.GroupBy(x => x.DistanceToCentroid)
                 .Select(s => new { Distance = s.Key, Count = s.Count(), List = s })
                 .ToList();

                foreach (var distanceGrouped in groupedByDistance)
                {
                    var greedyReduction = new GreedyReduction(distanceGrouped.List.ToList());
